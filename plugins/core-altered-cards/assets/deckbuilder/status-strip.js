...
(function() {
    ...
    function buildStrip() {
        ...
        placeStrip();
        window.matchMedia('(min-width: 992px)').addEventListener('change', placeStrip);
    }
    function placeStrip() {
        var host = document.getElementById('db-ss-sidebar-host');
        if (window.matchMedia('(min-width: 992px)').matches && host) {
            host.appendChild(strip);
            strip.classList.add('in-panel');
        } else {
            var layout = document.querySelector('.db-layout');
            layout.parentNode.insertBefore(strip, layout);
            strip.classList.remove('in-panel');
        }
        syncStickyOffsets();
    }
    ...
    function syncStickyOffsets() {
        ...
        if (strip) {
            var stripH = strip.classList.contains('in-panel') ? 0 : strip.offsetHeight;
            document.documentElement.style.setProperty('--db-ss-strip', stripH + 'px');
        }
    }
